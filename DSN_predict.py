...
def predict_img(iter,total,file,net,image_name,root_path,pred_file):
    ...
    output = class_label.data.max(1, keepdim=True)[1]
    output = output.cpu()
...
def main():
    ...
    for i in filenames:
        if(predict_img(iter,total,file,net,i,root_path,pred_file)):
